...
export default function VendorForm({ vendorId, initialData, isEditing = false }: VendorFormProps) {
  ...
  useEffect(() => {
    if (isEditing && initialData) {
      setFormData({
        ...
        vk: initialData.vk || '',
        is_venue: initialData.is_venue || false,
        venue_type: initialData.venue_type || '',
        seated_capacity: initialData.seated_capacity || 0,
        standing_capacity: initialData.standing_capacity || 0,
        max_capacity: initialData.max_capacity || 0,
        dressing_rooms: initialData.dressing_rooms || 0,
      ...
      if (initialData.service_cities) {
        const custom = initialData.service_cities.filter(
          (city: string) => !DEFAULT_CITIES.includes(city)
        );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        alert(`✅ Вендор ${isEditing ? 'обновлен' : 'создан'} успешно!`);
        router.push('/admin/vendors');
      } else {
        alert(`❌ ${responseData.error || `Ошибка при ${isEditing ? 'обновлении' : 'создании'} вендора`}`);
      }
    } catch (error) {
      console.error('Error:', error);
      alert(`❌ Ошибка при ${isEditing ? 'обновлении' : 'создании'} вендора`);
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-4xl font-bold text-gray-900">
            {isEditing ? '✏️ Редактировать вендора' : '➕ Создать вендора'}
          </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {step === 5 && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8 space-y-6">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-900 mb-2">
                  {isEditing ? 'Текущий статус' : 'Начальный статус'}
                </label>
                <select
                  name="status"
          ...
          <div className="flex gap-4 justify-between">
            ...
            {step < totalSteps ? (
              ...
              <Button loading={loading} type="submit">
                {isEditing ? '✏️ Сохранить изменения' : '✅ Создать вендора'}
              </Button>
            )}
          </div>
        </form>
